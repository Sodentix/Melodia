...
function handleCrop() {
  ...
  outCtx.save();
  outCtx.beginPath();
  outCtx.arc(radius, radius, radius, 0, Math.PI * 2);
  outCtx.closePath();
  outCtx.clip();

  outCtx.translate(outputSize / 2 + position.value.x * (outputSize / size), outputSize / 2 + position.value.y * (outputSize / size));
  outCtx.scale(scale.value * (outputSize / size), scale.value * (outputSize / size));
